...
var server = http.createServer(function (request, response) {
  ...
  if (path === "/index.html" || path === "/") {
    ...
    let page = JSON.parse(fs.readFileSync("db/page1.json".toString()));
    let s = string + "<ul id='list'>" + page.map((item) => `<li>${item.id}</li>`).join('') + "</ul>"; 
    response.write(s);
    response.end();
    ...
    response.end();
  } else if (path === "/style.css" || path === "style") {
    response.statusCode = 200;
